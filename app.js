...
function renderLeaderboard() {
  ...
  document.getElementById('leaderboard-list').innerHTML = rows.map((d, i) => `
    <div class="leader-row ${d.you ? 'partner' : ''}">
      ...
      <div style="font-size:12px; color:var(--ink-soft); font-family:var(--font-mono);">${d.streak}d streak</div>
    </div>`).join('');
}
...
function renderPartners() {
  ...
  const rows = partners.map(p => `
    <div class="leader-row">
      ...
      <div style="font-size:12px; color:var(--ink-soft); font-family:var(--font-mono);">${p.streak}d streak</div>
    </div>`).join('');
